...
-(void)	awakeFromNib
{
	...
	// Keep the original rectangle for later use
	[[tabView window] center];
	windowSize = [[tabView window] frame].size;
...
/* -----------------------------------------------------------------------------
	resizeWindow:
		Added for delx by iTerm so that panes autoresize in height
   -------------------------------------------------------------------------- */
...
-(void) resizeWindow
{
	...
	float lowest = -1;
	float diff = 0;
	NSRect newFrame = [[tabView window] frame];

	it = [[[[tabView selectedTabViewItem] view] subviews] objectEnumerator];
	while(view = [it nextObject]) {
		if([view frame].origin.y < lowest || lowest < 0) {
			lowest = [view frame].origin.y;
		}
	}

	diff = (newFrame.size.height - windowSize.height) + lowest - 10;
	newFrame.origin.y += diff;
	newFrame.size.height -= diff;
	[[tabView window] setFrame:newFrame display:YES animate:YES];
}
